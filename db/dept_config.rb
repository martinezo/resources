...
Catalogs::Department.select(:id).each do |dept|
    if dept.id > 0
      ...
      Admin::DeptConfig.find_or_create_by(department_id: dept.id, tag: 'reservations_datetime_op_1', label: 'Campo opcional de fecha/hora 1 (Solicitudes). Título:', precedence: 110)
      Admin::DeptConfig.find_or_create_by(department_id: dept.id, tag: 'reservations_datetime_op_1_hint', label: 'Nota para campo opcional de fecha/hora 1 (Solicitudes):', precedence: 120)

      Admin::DeptConfig.find_or_create_by(department_id: dept.id, tag: 'reservations_datetime_op_2', label: 'Campo opcional de fecha/hora 2 (Solicitudes). Título:', precedence: 130)
      Admin::DeptConfig.find_or_create_by(department_id: dept.id, tag: 'reservations_datetime_op_2_hint', label: 'Nota para campo opcional de fecha/hora 2 (Solicitudes):', precedence: 140)
    end
end
